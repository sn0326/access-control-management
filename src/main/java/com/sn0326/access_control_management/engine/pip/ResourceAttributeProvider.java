...
/**
 * リソースの属性を解決するプロバイダのインタフェース。
 *
 * <p>このコンポーネントを利用するアプリケーションは、このインタフェースを実装した
 * Bean を Spring コンテナに登録することで、独自のリソース属性解決ロジックを注入できる。
 * URN 形式（例: {@code urn:myapp:document:123}）の識別子を使うことで、
 * リソース種別ごとに異なるプロバイダへルーティングできる。
 *
...
public interface ResourceAttributeProvider {
    ...
    /**
     * 指定されたリソース識別子をこのプロバイダが処理できるか判定する。
     *
     * @param resourceId リソース識別子（数値ID、URN 等）
     * @return 処理できる場合 {@code true}
     */
    boolean supports(String resourceId);
     ...
     * と一致させること。
     *
     * @param resourceId リソース識別子
     * @return 属性マップ（key=属性名, value=文字列化した値）
     */
    Map<String, String> resolve(String resourceId);
}
